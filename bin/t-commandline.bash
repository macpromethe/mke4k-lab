...
Commands:
  deploy lab          Provision instances + install MKE4k (full deployment)
  deploy instances    Provision EC2 instances and NLB only (terraform apply)
...
case "${COMMAND}" in
    deploy)
        case "${SUBCOMMAND}" in
            lab)       cmd_deploy_lab ;;
            instances) cmd_deploy_instances ;;
            cluster)   cmd_deploy_cluster ;;
